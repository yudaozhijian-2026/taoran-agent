...
独立运行的TAORAN拜访评分Agent，版本`0.15.0`。已整合DSM知识库TAORAN标准，以及销售管理智能评测40题的Q33和Q34：
...
`feedback_text`继续等于`rule_feedback_text`，用于兼容旧简道云输出映射。三份反馈都不阻断提交、不生成提交前正式分数。知识库AI语义整理失败时，`knowledge_feedback_text`立即返回已经完成的结构化知识反馈；知识API本身不可用时仍明确提示接口异常，不用本地快照冒充实时知识。
