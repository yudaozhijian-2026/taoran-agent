...
独立运行的TAORAN拜访评分Agent，版本`0.15.1`。已整合DSM知识库TAORAN标准，以及销售管理智能评测40题的Q33和Q34：

- Q33记录完整性与及时性：50分；
- Q34自评一致性与下一行动：50分；
- 综合满分：100分；
- 提交前AI检测只给修改建议，永不阻断提交；
- 保留一个“AI检测”按钮，每次点击同时返回规则和实时知识库两份反馈；
- 规则反馈保持原`T—A—O/KR—R—A—N`六项本地检查逻辑不变；
- 知识库反馈先在本地完成结构化知识判断，再由当前智谱模型使用短提示只自然化未达标项；达标项不生成长篇分析；
- 前端结构化检查目标1秒内、语义整理最长6秒、按钮总目标约8秒；语义失败立即保留结构化反馈；
- 相同租户、输入、知识版本与模型提示命中摘要缓存时复用语义结果，默认缓存5分钟；
- 已取消独立“AI反馈意见（大模型反馈）”的前端生成、接口返回和提交后回写；
- 提交后重新生成知识库反馈并回写独立字段；深度评分证据校验失败时，知识库反馈仍可独立保留；
...
`feedback_text`继续等于`rule_feedback_text`，用于兼容旧简道云输出映射。两份反馈都不阻断提交、不生成提交前正式分数。知识库AI语义整理失败或排队超时时，`knowledge_feedback_text`立即返回已经完成的结构化知识反馈；知识API本身不可用时仍明确提示接口异常，不用本地快照冒充实时知识。旧`feedback_mode=ai`请求返回410，明确提示纯大模型反馈已取消。
