...
VERSION = '1.0.3-token-usage-20260911'
OLD = '1.0.2-auto-update-20260909'
EXPECTED_IMAGE = 'sha256:a052947c8e0cfa417224e10335bbff21bd889b01aa0b40760c804f70091153e0'
EXPECTED_REVISION = 'b36bab9bc439946f51d370b04080c753ac44fed6'
...
def expected_old():
    ...
    assert health()['release_version'] == '1.0.2'
    return c
...
def main(mode):
    ...
    if mode == 'preflight':
        ...
        assert h['release_version'] == '1.0.3' and h['prewarm']['status'] == 'ready'
        assert c['Config']['Labels']['org.opencontainers.image.revision'] == (RELEASE / 'git-commit.txt').read_text().strip()
        assert health('https://taoran.yudaozhijian.top')['release_version'] == '1.0.3'
        ...
        ledger = json.loads(run(['docker', 'exec', CONTAINER, 'python', '-c', code]))
        report = {'release': VERSION, 'verified_at': datetime.now(timezone.utc).isoformat(), 'image_id': c['Image'],
        ...
        temporary = BASE / 'current.token-usage-new'
