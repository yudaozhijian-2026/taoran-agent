...
VERSION = "0.27.19rc1-ui-feishu-20260908"
OLD = "0.27.18rc1-suggestion-contract-20260908"
EXPECTED_IMAGE = "sha256:19bd237d11b25e855265663e0dd31d2c7537a3d34c4ecf8b996be282d9f4a80a"
...
def main(mode):
    ...
    if mode in {"preflight", "switch"}:
        assert inspect()["Config"]["Labels"].get("org.opencontainers.image.revision") == (
            "578e666b76cfa354dcabca74264cefd5b07c2b2b"
    ...
    if mode == "preflight":
        ...
        assert health()["release_version"] == "0.27.18rc1"
        ...
        pending = (RELEASE / "pending-alerts.env").read_text()
        assert all(line.startswith('DSM_TAORAN_FEISHU_') for line in pending.splitlines() if line)
        current = (BASE / "runtime/agent.env").read_text()
        updated = '\n'.join(line for line in current.splitlines()
                            if not line.startswith('DSM_TAORAN_FEISHU_')) + '\n' + pending
        env = BASE / "runtime/agent.env"
        # Preserve ownership/mode for the unprivileged application container.
        ...
        h = health()
        assert h["release_version"] == "0.27.19rc1" and h["prewarm"]["status"] == "ready"
        ...
        assert db['counts'].get('feishu_alert_outbox', 0) == 0, 'Historical alerts unexpectedly queued'
        ...
        result = {
            ...
            "source": source,
            "database": db,
            "configuration_change": "runtime/agent.env: approved Feishu notification settings only",
