...
POLICY_VERSION = "TAORAN-POST-REVIEW-20260907-V4.4"
POLICY = (
    "2026-09-07公司确认：事实表达冲突允许一次受控重核，重新读取原始证据并修正意见，"
    "必须依据本次最新提交的拜访记录重新判断并计算评分，不沿用首次模型判断或历史评分。"
    "事实冲突重核须重新生成全部六项与评分依据，不能只修改文字而保留被拒绝的判断。"
    "主体不明确时保留不明确，不能猜成客户或销售；只有影响具体判断时才建议核实。"
    "等待、计划、正在和完成分别表达；等待客户评估不能写成正在或已经评估。"
    ...
    "主体判断遵循实质影响原则：先判断是谁做了什么是否清楚，再判断歧义是否改变具体结论。"
    "客户表示预算尚未批准已经明确表达方是客户，不因没有姓名或职务要求补充。"
    "现场给客户收货不能证明客户已经签收；仅当需要据此判断签收或目标达成时，"
    "建议说明是谁完成收货、客户是否实际确认。"
    "例如以客户认可采购方案为判断目标时，可反馈：已确认采购方案没有说明是谁确认的，"
    "目前还不能据此判断客户已认可方案，请补充实际确认方及确认内容。"
    "自评是达到目的、部分达到或未达到的固定选项，按原定目标达成判断比较；"
    "确认某状态与促成该状态是不同目标，例如确认预算未获批也可达成确认预算状态的目标。"
    "自评与实际达成一致且依据已清楚时，不以自评文字不详细或未解释业务差异要求改写固定选项，"
    "不因此将A2判为needs_revision；必要的业务区别可在分析中简短解释。"
    "局部主体歧义不自动使整条记录未达标，不自动将process_fact_based置为false或purpose_achievement置为not_achieved；"
    "分别保留其他明确事实的证明力，仅对受影响的具体结论说明证据不足，并依据原定目标重新判断达成程度。"
    "若已有明确事实足以支撑当前判断，而另一处主体歧义不影响该判断，不追加主体核实、姓名或职务填写要求，也不必复述这处无关歧义；不影响判断并不允许将该动作猜成销售所做。"
    "fact_source_unclear须在decision_impact说明哪一具体结论因主体不明无法确定；不能用记录不完整或主体未写作泛化理由。"
    "客户下一步明确共识仅适用于商机客户；潜力客户及目标客户缺少下一步承诺不构成缺口，"
    "facts.reason和各项建议同样必须遵守此边界。"
    "日期跨月、跨季度及先后关系使用authoritative_checks.calendar的程序计算结果，不自行推翻。"
