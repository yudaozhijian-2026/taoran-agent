...
def test_active_copy_mapping_writes_rule_and_knowledge_feedback_fields(monkeypatch) -> None:
    ...
    monkeypatch.setattr("taoran_agent.writeback.httpx.post", fake_post)
    request = evaluation_request()
    evaluation = TaoranAgent().evaluate(request, "job-two-feedback-writeback")
...
def test_failed_deep_review_still_writes_knowledge_feedback_field(monkeypatch) -> None:
    captured = {}
    ...
    assert result.status == "failed"
    assert set(captured["body"]["data"]) == {"_widget_1787803259012"}
    assert set(result.written_fields) == {"_widget_1787803259012"}
