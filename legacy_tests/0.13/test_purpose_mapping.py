...
def test_confirmed_synonyms_and_p5_contract_signing_are_allowed() -> None:
    mapping = structure_purpose_mapping(mapping_record())

    p1 = purpose_policy_for_visit(mapping, visit(opportunity_stage="P1"))
    p2 = purpose_policy_for_visit(mapping, visit(opportunity_stage="P2"))
    p3 = purpose_policy_for_visit(mapping, visit(opportunity_stage="P3"))
    p5 = purpose_policy_for_visit(mapping, visit(opportunity_stage="P5"))

    assert {"获得参与", "获得参与资格"} <= set(p1.allowed_purposes)
    assert {"技术方案认可", "认可技术方案", "获得提方案的机会"} <= set(
        p2.allowed_purposes
    )
    assert {"击败竞争对手", "赢得商务竞争", "许可参与商务谈判的机会"} <= set(
        p3.allowed_purposes
    )
    assert {"协助项目实施", "完成合同签署"} <= set(p5.allowed_purposes)
    assert "争取客户满意" not in p5.allowed_purposes
