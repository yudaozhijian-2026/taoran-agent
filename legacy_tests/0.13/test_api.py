...
def test_model_button_still_returns_advice_only(monkeypatch):
    ...
    assert "model_feedback_text" not in body
    assert len(calls) == 0


def test_button_returns_rule_and_live_knowledge_model_feedback(monkeypatch):
    from test_llm import reviewer_for
    ...
    payload = complete_precheck_payload("api-two-feedbacks")
    response = TestClient(api.app).post(
        "/api/v1/connectors/jiandaoyun/visit/button-check",
        json={"context": payload["context"], "form_data": payload["visit"]},
    )
    ...
    assert {item["id"] for item in body["live_knowledge_references"]} == {
        "DSM-BS-01-06", "DSM-BS-000", "DSM-BS-01-07",
    }
    assert body["official_score_generated"] is False
    assert len(calls) == 1
    prompts = [call[1]["messages"][0]["content"] for call in calls]
    assert all("LIVE-KNOWLEDGE-API-CONTENT" in prompt for prompt in prompts)
...
def test_single_button_starts_rule_and_knowledge_branches_in_parallel(
    monkeypatch,
    tmp_path,
):
    from taoran_agent.agent import TaoranAgent
    ...
    request = PrecheckRequest.model_validate(complete_precheck_payload("parallel-branches"))
    baseline = TaoranAgent().precheck(request)
    rule_started = Event()
    knowledge_started = Event()

    def run_rule(request, agent):
        del request, agent
        rule_started.set()
        assert knowledge_started.wait(1)
        sleep(0.03)
        return baseline

    def run_knowledge(request, settings, snapshot=None):
        del request, settings, snapshot
        knowledge_started.set()
        assert rule_started.wait(1)
        sleep(0.03)
        return baseline

    monkeypatch.setattr(api, "_execute_precheck_with_agent", run_rule)
    monkeypatch.setattr(api, "_execute_live_knowledge_feedback", run_knowledge)
    settings = Settings(database_path=str(tmp_path / "parallel.db"), _env_file=None)

    rule, knowledge, rule_ms, knowledge_ms = api._execute_two_feedback(request, settings)

    assert rule == baseline
    assert knowledge == baseline
    assert rule_started.is_set() and knowledge_started.is_set()
    assert rule_ms >= 30
    assert knowledge_ms >= 30
...
def test_button_t03_uses_exact_knowledge_mapping_and_rejects_p6(monkeypatch):
    ...
    knowledge_prompt = prompts[0]
    assert "潜力客户：收集信息" in knowledge_prompt
    assert "P6争取客户满意" in knowledge_prompt
...
def test_rule_t03_uses_published_snapshot_when_live_knowledge_is_unavailable(monkeypatch):
    monkeypatch.setenv("DSM_TAORAN_KNOWLEDGE_API_KEY", "synthetic-knowledge-key")
    get_settings.cache_clear()
    ...
    payload["visit"].pop("purpose_policy")
    payload["visit"]["purpose_code"] = "击败竞争对手"
    ...
    body = response.json()
    assert calls == 0
    assert "TAORAN_T03_PURPOSE_MAPPING_UNAVAILABLE" not in {
        item["code"] for item in body["issues"]
    }
    assert "AI调用异常：知识库接口调用失败" not in body["feedback_text"]
    stored = api.get_store().get_precheck("tenant_demo", body["check_id"])
    assert stored["request"]["visit"]["purpose_policy"]["source_knowledge_id"] == (
        "DSM-BS-01-06"
    )
...
def test_repeated_date_anomaly_then_correction_refreshes_button_feedback():
    ...
    payload.pop("purpose_policy", None)
    payload["purpose_code"] = "击败竞争对手"
...
def test_plugin_subforms_reach_agent_and_updates_replace_old_values(monkeypatch):
    ...
    hashes = set()
    purposes = {'P3': '击败竞争对手', 'P4': '完成合同签署', '': '收集信息'}
    for stage in ['P3', 'P4', '']:
        draft["purpose_code"] = purposes[stage]
