...
def test_precheck_model_has_fast_button_defaults() -> None:
    settings = Settings(_env_file=None)

    assert settings.llm_precheck_timeout_seconds == 5
    assert settings.llm_precheck_max_output_tokens == 1200
    assert settings.llm_max_output_tokens == 3000
    assert settings.knowledge_timeout_seconds == 1.5
    assert settings.knowledge_snapshot_cache_seconds == 300
